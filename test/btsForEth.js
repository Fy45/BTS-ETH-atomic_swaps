...
async function btsForEth() {
  ...
  const Secret = new Buffer.from(secret).toString('hex');
  ...
  console.log("Please inform your counter party with the hash value: ", hash_lock);
  ...
  let id = await prompt('Enter the your recipient account id of ETH wallet: ')
