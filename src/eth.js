...
const htlcArrayToObj = require("./helper/utils").htlcArrayToObj;
...
async function deployHTLC(
  ...
  const txReceipt = await htlc.methods
    .newContract(recipient, hash, timeLock)
    .send({
      from: sender,
      value: amountWei,
      gas: 3000000
...
async function verifyHTLC(mnemonic, api_key, contractId) {
  ...
  console.log(
    `Unlock time     | ${unlockTime} (~ ${Math.max(
      0,
      Math.floor((Number(contract.timelock) - nowSeconds()) / 60)
    )} mins)`
  );
...
async function resolveHTLC(mnemonic, api_key, receiver, contractId, secret) {
  ...
  const txGas = (txReceipt, gasPrice = defaultGasPrice) =>
    web3.utils.toBN(txReceipt.gasUsed * gasPrice);
  ...
  const withdrawTx = await htlc.methods
    .withdraw(contractId, secret)
    .send({ from: receiver, gas: 3000000 });
