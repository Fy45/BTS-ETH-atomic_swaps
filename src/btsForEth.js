...
async function btsForEth() {
  ...
  const btsHtlcid = await bts.deployHTLC(
    btsSender,
    btsRecipient,
    hash_lock,
    value,
  ...
  console.log(
    `Please inform your counterparty with the hash value: ${hash_lock}`
  );
  ...
  switch (answer) {
    case "yes":
      ...
      console.log(`Account: ${ethRecipient} has balance of ${balance} ETH`);
      break;
    case "exit":
      console.log("Exiting...");
      break;
